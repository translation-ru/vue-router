...
Либо, если вы используете сервер Node.js, вы можете внедрить поддержку 404 ошибки, используя маршрутизатор на сервере для сопоставления входящего URL и ответа с ошибкой 404, если маршрут не был найден. Подробнее ознакомьтесь с документацией [по серверному рендерингу Vue](https://vuejs.org/guide/scaling-up/ssr.html) для дополнительной информации.
